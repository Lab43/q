...
const check = (contents) =>
  runScript(stageFrontmatter({ "doc.md": contents }), "check-frontmatter.mjs");
...
describe("directories it must not walk", () => {
  ...
  it("ignores markdown inside node_modules", () => {
    ...
    assert.equal(runScript(base, "check-frontmatter.mjs").status, 0);
  });
  ...
  it("still walks .claude outside worktrees", () => {
    ...
    assert.equal(runScript(base, "check-frontmatter.mjs").status, 1, "only worktrees are excluded, not all of .claude");
  });
});
...
describe("reporting", () => {
  ...
  it("counts every markdown file it walked, not only those with frontmatter", () => {
    ...
    const { stdout, status } = runScript(base, "check-frontmatter.mjs");
  ...
  it("reports every failing file, not just the first", () => {
    ...
    const { stderr, status } = runScript(base, "check-frontmatter.mjs");
