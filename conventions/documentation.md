...
**Every doc this policy governs must stay reviewable by a human.** Deletion is the default and it is safe — git history keeps everything cut. When a judgment call between keeping and cutting is close, cut; when text is ambiguous or wrong, try deleting it before qualifying it — a clause bolted on as a patch is accretion in miniature. And new rules refine, never append: integrate a rule into the section it belongs to, rewriting that section so the doc reads as if written that way from the start — a doc must never grow a rulings changelog at the bottom; git history is the changelog. All are standing corrections to the model bias toward long prose and accretion; when any rule below leaves a close call, resolve it against that bias.
...
**Precedence: on conflict, project conventions win.** A project overrides a framework rule by recording the deviation in the project conventions doc that mirrors the framework doc it deviates from — a principles deviation in the project's `docs/conventions/principles.md` (created if needed), a documentation-policy deviation in its `documentation.md` — so the override sits where a reader applying the rule will look. A deviation is written as an ordinary rule under the topic it governs, a sentence or two: the decision and the rationale, plus an overrides marker (see: Markers) to keep it greppable — and it is refined in place or deleted as the decision evolves, never appended as a log entry. An override outlived by the framework — its target updated to agree, or gone — is spent and comes out. No other override mechanism exists or is needed — the readers are agents, so a stated deviation is the mechanism.
...
- **`docs/*.md`** — guides: instructions for using and operating the product, not for writing its code (deployment walkthroughs, feature guides, operational procedures). Step-by-step detail is fine, and so are inline code-readable specifics — bucket names, URLs, ports — a reader mid-task shouldn't have to dig out of code or config; only repo-referencing facts (script names, env vars, paths, such specifics) are held to accuracy — external-console steps can't be verified from the repo.
- **`README.md`** — the human overview. Summarizing facts owned elsewhere is its normal mode, not a violation — the obligation is checkability: a summary that mirrors one identifiable home carries a source marker; free-form overview prose is held accurate against the things it describes by grooming. Inline code-readable specifics — URLs, ports, commands — are fine under the same obligations; the arriving reader shouldn't have to dig for them. Content answers an arriving reader's questions — what this is, what it does, how to use it. Enumerating the product's interface (commands, skills, entry points) serves that and belongs; inventorying the repo's internals (directory layout, file lists) restates what browsing already shows — an internal detail earns mention only when it explains something non-obvious to the reader. Prose is evergreen: a sentence describing the current moment ("being migrated to…") rots silently once the moment passes — describe what the product is, and let git history carry the journey. The README and the agent briefing never point at each other — a pointer sends a reader to a document written for a different reader.
- **`AGENTS.md`** (or `CLAUDE.md`) — the always-loaded agent briefing; every line costs context in every session. It carries only what applies session-wide, plus the docs index — every conventions doc, and any guide useful to agent sessions, one line per doc, restating the doc's intro (skills need no briefing index — the session's skill list already carries every skill's name and description). Information needed only for particular kinds of work lives in the relevant convention doc or skill, with at most a one-line pointer here.
