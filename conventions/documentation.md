...
**Precedence: on conflict, project conventions win.** A project overrides a framework rule by recording the deviation in the project conventions doc that mirrors the framework doc it deviates from — a principles deviation in the project's `docs/conventions/principles.md` (created if needed), a documentation-policy deviation in its `documentation.md` — so the override sits where a reader applying the rule will look. A deviation is written as an ordinary rule under the topic it governs, a sentence or two: the decision and the rationale, plus an overrides marker (see: Markers) to keep it greppable — and it is refined in place or deleted as the decision evolves, never appended as a log entry. No other override mechanism exists or is needed — the readers are agents, so a stated deviation is the mechanism.
...
- **`AGENTS.md`** (or `CLAUDE.md`) — the always-loaded agent briefing; every line costs context in every session. It carries only what applies session-wide, plus the docs index — conventions docs and guides, one line per doc, restating the doc's intro (skills need no briefing index — the session's skill list already carries every skill's name and description). Information needed only for particular kinds of work lives in the relevant convention doc or skill, with at most a one-line pointer here.
