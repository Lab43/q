...
q's documentation keeps every fact in exactly one authoritative home, but text still needs to point at, copy, or disagree with facts that live elsewhere. One site may also need to sit outside a rule the rest of the project follows. Markers declare which of those relationships is in play — making them visible to readers and checkable by grep, with no central list to maintain:
...
| `(exception: X)` | This one site sits outside the named rule, which still stands everywhere else (`exception: docs/conventions/testing.md, What carries tests`). The reason is the text the marker sits in, in prose or in a code comment. `/q:groom-docs` counts them by rule, so several against one rule surface as a signal the rule wants revisiting. |
