...
- **The pack's `conventions/`** — hold the project's docs against each changed rule:
  ...
  - a project rule the new text contradicts is the one call the go-ahead didn't settle — ask: keep it as a recorded deviation (add the overrides marker) or adopt the framework rule. Adopting can leave code non-conforming — suggest `/q:review` on the affected area, conventions lens; code fixes are out of scope here
