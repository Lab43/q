...
- On an extension run:
  ...
  - Any exception markers the installed package carries against q's rules — grep the package for `exception: [^,)]*\.md`, not only its `conventions/`, because an exception can sit in a code file. These are single sites of the extension's own that sit outside a rule the project otherwise follows. The project cannot edit them, and grooming's count never reaches them.
