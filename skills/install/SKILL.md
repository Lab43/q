...
- On an extension run:
  ...
  - Any exception markers the installed package carries against q's rules — grep the package, not only its `conventions/`, because an exception can sit in a code file. These are single sites the project now lives with outside a rule it otherwise follows.
