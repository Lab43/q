...
- **Pinned behind latest** → diff the two versions' `conventions/` (download both with `npm pack <pack>@<version>` into a scratch directory and extract), summarize what changed and what reconciliation it demands of this project, then ask: move the pin (Steps 2–4), or stay and sync — the go-ahead that makes the rest of the run autonomous. The pin is a recorded decision — only the user moves it.
