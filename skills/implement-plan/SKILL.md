...
Clarify the plan with the user, in conversational mode (see: ${CLAUDE_PLUGIN_ROOT}/references/collaboration.md, Collaboration modes) — everything asked here is a question the autonomous stretch won't have to stop for. One batch, via AskUserQuestion:
...
- The review mode, always asked (see: ${CLAUDE_PLUGIN_ROOT}/references/collaboration.md, Review modes): in this run, ship covers every commit (a stacked run's `gh stack` operations included), push, and PR; local commits nothing until the user has reviewed it, each PR's content at that PR's boundary.
...
2. **Final review**: launch two `adversarial-reviewer` subagents in parallel over the PR's diff — one with the **correctness** lens, one with the **conventions** lens. Give each the plan path, which plan steps this PR delivers (and that the rest live in other PRs), and the diff scope. Then loop, at most three times: fix the BLOCKING findings; re-run the checks covering the fixed code, re-exercising any flow from item 1 that a fix changed; in ship mode, commit the round. Review again with the same two reviewers. The loop exits when neither reports a BLOCKING finding; findings that survive the cap become Caveats in the PR description.
3. **Local review's gate**: stop and ask the user to review the PR — the uncommitted diff, presented with its check results and anything else they should weigh. Expect change requests: make them and iterate with the user, running no machinery per exchange. At their go-ahead, commit exactly what they reviewed onto the PR's layer. Then run the checks covering what the session changed and, when the changes were substantive, one `adversarial-reviewer` pass (both lenses) over them. Never fold the resulting fixes into the reviewed commit: leave them uncommitted and return to the gate, where the user reviews them as their own diff and may ask for more changes. Repeat until a go-ahead leaves nothing uncommitted.
