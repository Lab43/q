...
3. **Local review's gate**: stop and ask the user to review the PR — the uncommitted diff, presented with its check results and anything else they should weigh. Expect change requests: make them and iterate with the user, running no machinery per exchange. At their go-ahead, run the checks covering what the session changed and, when the changes were substantive, one `adversarial-reviewer` pass (both lenses) over them — then commit the PR onto its layer as one commit.
