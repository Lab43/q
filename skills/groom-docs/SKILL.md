...
`docs/conventions/` is the surface to groom. Read rubric item 1's docs where their path form resolves (see: @lab43/q conventions/documentation.md, Package doc paths). Stop and suggest the fix when either is missing. A fresh clone may just need `npm install`.
