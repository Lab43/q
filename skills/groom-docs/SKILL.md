...
7. **Exception accumulation**: grep the repo for "exception:", excluding `node_modules/` and build artifacts, and group the hits by the rule each names. The parentheses are dropped inside a comment, so a grep requiring them misses every marker in a code file (source: @lab43/q conventions/documentation.md, Markers). Skip the text that defines the marker and the examples illustrating it. Report every rule carrying more than one, for the user's ruling — several exceptions against one rule are evidence the rule wants revisiting (source: @lab43/q conventions/documentation.md, Markers). Report a marker naming no section, one carrying no reason, and one whose reason the surrounding code or doc no longer bears out. A file outside the grooming surface joins the run for its exceptions alone: nothing else in it is groomed.
