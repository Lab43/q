---
name: update
description: Update the project's installed extensions, q among them — move pins to the latest releases with the user's go-ahead, reconcile the project's docs with what each release changed, and catch up any pin that moved out of band. Invoked bare it covers every installed extension; a named target — q, or another extension — scopes the run. Use after a release ships, or whenever pins may be behind. To audit docs without updating, use groom-docs; to repair this machine without touching docs, use sync. A pin move or catch-up ships as a PR.
---
...
Bare invocation covers every installed extension — the direct `devDependencies` whose own `package.json` carries the `q-extension` keyword (source: q conventions/extensions.md), `@lab43/q` among them. A named target scopes the run: `q` means `@lab43/q`; any other name means that extension. Confirm any target you can't identify as an extension before treating it as one.

Read four versions for each extension in scope — the watermarks per `${CLAUDE_PLUGIN_ROOT}/references/q-state.md`:
...
An extension with no pin and no watermark entry has nothing to update — propose `/q:install` for it and stop.

Validate the records before sorting. Check that every pinned extension in scope carries its watermark. Check that no watermark outlives its pin: every watermark on a bare run, the target's on a named run. On any failure, propose `/q:sync` and stop.

Report the versions, then sort each extension by its state:

- **Pinned behind latest** → a pin move to offer. Diff the two published versions: `npm pack <extension>@<version>` for each into a scratch directory, extract both, and diff the trees. Diff the whole tarball rather than `conventions/` alone, because a release can change an extension's skills, hooks, agents and references too. Step 4's reconciliation and the closing report both read from this diff. Summarize what changed and what reconciliation it demands. Pins are recorded decisions — only the user moves them.
...
- **Changed `conventions/`** — hold the project's docs against each changed rule:
  ...
  Then sync the briefing's index lines for the extension — a doc added or removed changes the list, a changed intro re-draws its blurb (see: q conventions/documentation.md, Taxonomy).
- **A changed plugin** — re-run `/q:install`, scoped to join this run's change: it is idempotent, creating what the new version's scaffold expects and correcting what has drifted from it. `@lab43/q` is the only extension whose plugin q installs.

After each extension's reconciliation, write its watermark per `${CLAUDE_PLUGIN_ROOT}/references/q-state.md`: its `reconciledAgainst` entry to its pinned version.
