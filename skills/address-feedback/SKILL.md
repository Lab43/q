...
- **Track resolutions**: keep a scratchpad note of each item — where the reviewer wrote it, its agreed disposition, and how it resolved. It feeds the replies and the closing report.
...
5. Set aside the PR author's own callout comments. A PR opened under these conventions carries them on the diff, posted from the same account the reviewer comments from and signed (source: q conventions/pull-requests.md, Diff comments). They are the author's prompts to the reviewer, not feedback to work.
6. Build the agenda from everything the fetch turned up — one item per inline thread, plus one per top-level comment and review summary that carries feedback. Merge items that share a root cause or answer each other. Record where each item was written, because that decides how it gets answered. Post the agenda — each item's gist in one line, in the order you propose to take them. No positions yet, and no edits.
