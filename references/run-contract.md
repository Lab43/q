...
Fix the BLOCKING findings, applying judgment on nits. Re-run the checks covering the fixes. Review again with the same reviewers — fixes are always re-reviewed. A round whose fixes were few and small may hand the next round to one reviewer carrying both lenses instead. In ship mode, commit each round. Loop at most three times. The loop exits when no reviewer reports a BLOCKING finding, and findings that survive the cap are reported as caveats.
