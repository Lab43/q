...
Send nothing else. A peer message carries only what keeps two sessions from duplicating or colliding over the same work. Every message surfaces in the user's session, so every message spends the user's attention.
...
- Acknowledgements, thanks, and unasked-for status are noise.
