...
- **Head each package's group with its description** — the package name, then the `description` from that package's `package.json` (source: @lab43/q conventions/documentation.md, Taxonomy). q's group comes first, then one group per installed extension. Restate the description as written. Shortening an unwieldy one is the package author's job. A package shipping no description gets a heading of its name alone.
