...
1. In `q-extension/agents/adversarial-reviewer.md`, the description at `:3`: ground the reviewer in the project's conventions and specs. *(deviation: the description also lost its account of the report's format, which a caller learns after invoking. Ruled at the local gate.)*
...
1. `q-extension/skills/create-plan/SKILL.md`, Step 1 item 2: read the specs governing the territory beside the conventions. Step 3: a plan's Context cites the governing spec sections as givens, and a plan that changes committed behavior schedules the spec edit in the phase that ships it (see: Decision 8). *(deviation: the rule that a plan's Context cites the governing spec sections is a format rule, so its home is the plans convention's Sections, and the skill restates it with a source marker. Ruled at the local gate.)*
