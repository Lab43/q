...
5. Rewrite the two briefing index lines in `CLAUDE.md`. Line 29's blurb still reads "the doc pack format" after Phase 3 repointed its path, and line 28's must restate `documentation.md`'s changed intro. An index line restates its doc's intro, so both follow the rewrites above rather than waiting for Phase 8's sweep. *(deviation: only line 29 moved. `documentation.md`'s intro never mentioned packs and this phase did not change it, so line 28 already restates it.)*
