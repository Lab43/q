...
3. Add a staging helper for it to `test/helpers.mjs`, and write `test/check-tests.test.mjs`. Cover an executable with its test, one without, one carrying a valid exception, one carrying a marker with no section, each skipped directory, and a `.husky/` file against `.husky/_/`. *(deviation: the copying the three staging helpers had converged on is extracted into one `stageScripts`, which edits `stageFrontmatter` and `stageVersions` — files no step here names. The third copy was the signal.)*
...
6. Break the check deliberately and confirm its suite goes red, once per branch. *(result: 47 mutations across all four scripts and the test harness, all caught. Seventeen target this check. Two pinned branches the first revision missed: a marker read out of a string literal, and the staged suite hiding the script from its own walk.)*
