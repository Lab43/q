...
6. Break each script deliberately and confirm the suite goes red, once per branch the suite claims to cover (source: docs/conventions/testing.md, Show the suite failing). Mutate the staging helper too. *(result: 24 mutations, all caught. Three of them pin branches an earlier revision left green: a lockfile written before the guard that rejects it, the version rewrite made global, and the lockfile's disagreement message reporting the wrong field. Four mutate the harness.)*
