...
Give q a way to excuse one site from a convention, and move the existence half of the testing rule onto `npm run check`. *(deviation: only the first half shipped. The check was built and then dropped in review — see the Phase 4 heading.)*
...
3. **One marker serves prose and code alike.** A site excused from a rule may be a passage in a doc or a line in a script, and the marker reads the same in both. `conventions/documentation.md:82` already sanctions a marker inside an HTML comment, so a code comment is the same move for a different file type. One form also means one grep: two forms would split the count that Decision 1 rests on. *(deviation: the law reaches any file, not code specifically. The gap this marker exists to close is a repo-wide MD041 disable in `.markdownlint-cli2.jsonc`, which is config. Stating the reach as code would have left the motivating site unexcusable.)*
...
7. **Both of the remaining untested sites take exceptions.** `docs/workflow-chart/screenshot.py` needs a browser that CI does not install, and a test could only assert that two images get written. `.husky/pre-commit` is one line delegating to `npm run check`, and a test could only assert the file contains that line. Testing either would bend the design around the tooling (source: @lab43/q conventions/principles.md, Tooling limitations never dictate content). *(deviation: bending the design around the tooling is not what excuses either site. A browser CI does not install is a tooling gap, which that same principle says to fix rather than accept. `docs/conventions/testing.md` therefore names a second ground, that the only test available would pin nothing, and `.husky/pre-commit` rests on it squarely.)*

   *(result: the conventions lens rejects `screenshot.py`'s ground. Given Chromium, a real test is available — `light.png` and `dark.png` must differ, which pins that `color_scheme` reached the render — so "the only test available would pin nothing" is false there. What actually stands in the way is that nothing in this repo installs Chromium, and the bar refuses a tooling gap. The site ships excused with a truthful reason, and whether it should be excused at all is the user's ruling. Widening the bar a second time to admit it would be the weaken-the-convention move this plan exists to remove. The accumulation is unchanged: two exceptions, one rule.)*

   This puts two exceptions against one rule the day the mechanism lands, which is the accumulation signal Decision 4 describes. That is left standing rather than pre-empted. Narrowing the rule now would settle by assumption what the mechanism exists to settle by evidence, and it would ship the marker with no first user. *(deviation: review narrowed the rule, and `.husky/pre-commit` takes no marker. The accumulation signal fired as this decision predicted, and the evidence it produced was that the two sites are not alike: the hook has no behavior to test, while `screenshot.py` has behavior an obstacle blocks. The first is the rule over-reaching, so it is fixed in the rule; only the second is an exception. One marker ships.)*
...
6. In `conventions/extensions.md`, Authoring: extend the line at `:42` so an extension's own sites may carry an exception to a rule it consumes, while a rule it authors is edited instead. *(deviation: the line was left as it stood, naming deviations, overrides, and upstreaming alone. Review rejected barring an author from excepting a site from a rule it ships. A deviation or an override restates law that competes with the rule you own, which is what the bar exists for. An exception annotates one site and restates nothing. The asymmetry the bar created has no grounding either — Phase 4 excepts two sites from `docs/conventions/testing.md`, a rule this repo authors. The Markers section restricts no tier, so silence permits and no rule was added.)*
...
*(deviation: review turned up two edits no step names. `conventions/documentation.md` gained the spent-and-retarget lifecycle the readers were acting on without a home. `docs/conventions/documentation.md`'s "Structure is earned" gate had stated the markers' bar as greppability alone, which is not the bar this marker was judged against. Three further edits were made and then reversed on the pull request: a grooming obligation in the Markers intro, which restated the counting rule the section already carried twice, a carve-out letting a marker example name an illustrative path, which the overrides bullet's own examples have done since before this plan without one, and a bar on targeting a heading in the current doc, which the mandatory section already entails.)*

#### Phase 2: Teach the readers

1. In `agents/adversarial-reviewer.md:31`, conventions lens: before reporting a site against a rule, read the site for an exception naming that rule. A marker carrying a reason answers the finding. A bare one does not — report it, because a marker with no reason is the convention being skipped rather than excused. Keep the sibling-grep clause, and record that a marked exception is the durable form of the evidence it hunts for. *(deviation: the marker instruction went into the shared Work review preamble instead, so both lenses read it. The correctness lens hunts missing test coverage on its own, and would have reported Phase 4's two marked sites with nothing answering it. The preamble also rules that a marker the work under review introduces is judged rather than honoured, closing the hole where a change excuses itself by adding one. Deference turns on whether the reason holds rather than on whether the marker is pre-existing, so a hollow excuse is reportable in either artifact the work review takes. Scoping it to a diff would have left a bogus marker unchallenged in the files-only artifact, whose whole job is to show what should not stand.)*
...
5. In `skills/update-docs/SKILL.md:30`, gate 2: when the lesson is that one site sits outside a rule, the comment it becomes carries an exception marker. *(deviation: gate 2 first gained the extension-authoring caveat as well, because gate 2 can exit before gate 4 is consulted and Phase 1 step 6 then banned an author from excepting a rule it ships. Reversing that ban took the caveat back out, leaving gate 2 as this step wrote it.)*
...
9. In `skills/install/SKILL.md:149`, the extension run's closing report: report the exception markers a newly installed extension carries alongside its overrides markers. An extension's sites can sit outside a q rule it consumes, and the project takes those on with it. *(deviation: the report was left reporting overrides alone. An overrides marker belongs there because it changes the law the project now operates under. An exception excuses one of the extension's own sites, changes no rule the project follows, and leaves the reader nothing to act on — the bullet said so itself. It also bought a whole-package grep of `node_modules/` for that. The accumulation gap it half-covered stands, unchanged by a notice shown once at install.)*

*(deviation: one edit no step names. `skills/update-docs/SKILL.md` gate 4 restates the `conventions/extensions.md` line under a source marker, so it had to follow that line wherever it went. It gained "exceptions" when Phase 1 step 6 put them there, and lost the word again when the pull request reversed that. What stays is "deviations", which the home line has always carried and the restatement had been missing.)*
...
*(deviation: the check was dropped in review and never merged. Steps 1, 2 and 3 are reverted; steps 4 and 5 stand, and are what this pull request delivers. #91's Proposal asks for the marker and nothing else, so the issue closes on that alone. What argued for a check was `conventions/principles.md`'s enforcement-rung rule, applied to a rule that no evidence shows prose failing to hold: the five violating executables were named in #92's own body and tracked in #94 before that pull request merged. `docs/conventions/documentation.md` asks for a plain-prose version to demonstrably fail first, and it has not. The contradiction that leaves — a mechanically checkable rule sitting on the weakest rung by choice — is real, and is the user's to resolve against the principle rather than against this plan.)*
...
3. Add a staging helper for it to `test/helpers.mjs`, and write `test/check-tests.test.mjs`. Cover an executable with its test, one without, one carrying a valid exception, one carrying a marker with no section, each skipped directory, and a `.husky/` file against `.husky/_/`. *(deviation: the step said to add a helper; it also rewrites the two that were already there. All three had converged on the same copying, so it is extracted into one `stageScripts`. The third copy was the signal.)*
4. Mark `docs/workflow-chart/screenshot.py` and `.husky/pre-commit` with exception markers and their reasons, per Decision 7. *(deviation: only `screenshot.py` is marked — see Decision 7.)*
5. Rewrite `docs/conventions/testing.md`, "What carries tests", rather than cutting a line out of it (source: @lab43/q conventions/writing.md, Refine rather than append). The existence obligation goes, because the check holds it now. What stays is the judgement the check cannot make: what a test has to exercise, and why verification that does not survive the session counts for nothing. Keep the heading — it is what the two exception markers target. *(deviation: the obligation stays, stated as a rule rather than deleted. The markers name this section, so there has to be a rule here to be excused from; what goes is any description of the check, whose rationale lives in the check itself. The section also states the grounds an exception needs, which the first draft got wrong — see Decision 7.)* *(deviation: the section names no exception and states no grounds. A rule does not announce that markers may excuse it, because that holds for every rule and its home is the Markers section. One of the two grounds was the rule over-reaching rather than a ground, and it became the rule's scope.)*
6. Break the check deliberately and confirm its suite goes red, once per branch. *(result: 54 mutations across all four scripts and the test harness, all caught. Twenty-three target this check. Six pinned branches earlier revisions missed: a marker read out of a string literal or off a line shared with code, an extension matched anywhere in a filename, a skip matching a directory that merely shares a name, an unsorted failure list, and the staged suite hiding the script from its own walk.)*

## Verification

- `npm run check` passes, with `check-tests` reporting two marked exceptions. *(deviation: the check was dropped, so nothing reports exceptions. `npm run check` passes without it.)*
- Delete `tests/check-versions.test.mjs` and confirm `check-tests` names it and fails. Restore it. *(deviation: not run. There is no check to name it.)*
- Strip the section from one of the two exception markers and confirm `check-tests` fails. Restore it. *(deviation: not run, for the same reason. Only one marker ships — see Decision 7.)*
- Run `/q:groom-docs` and confirm its accumulation check reports `docs/conventions/testing.md`, What carries tests, as carrying two exceptions. *(deviation: one marker ships, not two — see Decision 7.)* *(deviation: the run's clarification settled this as the grep Step 2 of the skill specifies, rather than a full grooming run. The skill audits the whole documentation surface and ships a pull request of its own, which is past what this plan delivers.)*
