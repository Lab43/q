...
8. **The format spec lives in the plugin.** `references/q-state.md` at the plugin root carries the file's format and writer rules, read by the three skills at run time. The consumer-facing guard is the in-file `note` plus one line in the briefing section install scaffolds. *(deviation: the briefing line was cut during PR review — the in-file note reaches exactly the reader about to edit the file, and a briefing line taxes every session for that rare moment.)* Rejected: a new pack conventions doc — a single rule does not pay for a doc plus its index line in every consumer briefing.
9. **install scaffolds the onboarding surfaces.** Two lines that travel with the repo, because sync ships inside the plugin and a fresh machine cannot self-bootstrap. For humans, a setup line in the consumer's README: the plugin install command, `npm install`, and `/q:sync` on each new machine. For agents, a briefing line: a session with no `/q:` skills in its skill list is on a machine missing the q plugin — hand the user the install command, then `/q:sync`. The briefing is the one agent-facing q surface that loads without the plugin, so prose is the strongest enforcement rung available there. *(deviation: the plan left the no-README case unspecified; the user's clarification at run start — install creates a minimal README carrying just the setup line when the project has none. During PR review the instructions became information-based rather than verbatim text, and gained registering the project marketplace before the plugin install — a fresh machine cannot resolve `q@q-pin` from the committed settings declaration alone, verified live.)*
10. **Hook checks.** Four comparisons, each silent when its inputs are missing:
    ...
    The last check is what tells a collaborator's fresh clone (plugin installed, `npm install` not yet run) that its conventions are unreadable. *(deviation: reworked on the user's direction during PR review. The hook validates the whole install — installed, pinned, and watermarked versions must agree for the plugin and every doc pack. Beyond the four comparisons it also fails on: a missing state file or `scaffoldedAgainst` on a pinned project; a `reconciledAgainst` entry for a pack no longer in `package.json`; and — a check the grep/sed version could not make — a devDependency whose installed manifest carries the `q-docs` keyword but has no watermark entry. Every failure emits one uniform message routing to `/q:sync`; the per-check messages and routing split were dropped to narrow the drift surface. sync hands never-recorded findings to `/q:install` (one run per pack), disagreeing records to `/q:update` bare (one sweep, one run), and — once issue #23's uninstall skill exists — removed-pack entries to it; until then update's bare sweep prunes them. A pack pinned but neither installed nor watermarked is invisible to the hook (no manifest to identify it by) until an npm install surfaces it.)*
...
- When the CLI does not know the `q-pin` marketplace — a scaffold from this same session: `claude plugin marketplace add ./.claude/q-marketplace`. *(deviation: the add runs `--scope local`, and the case is any machine before its first trusted session in the clone — a trusted session registers the declared marketplace itself, but nothing resolves `q-pin` before one, and the default user scope records one absolute path machine-wide that the next q project's add silently replaces; all verified live during PR review.)*
...
Extend `hooks/session-start.sh` with the three remaining Decision 10 comparisons: parse `.claude/q-state.json` with the same grep/sed technique the script already uses, and loop over the `reconciledAgainst` entries. Aggregate every finding into one message — the hook emits a single JSON document, so multiple findings share one `additionalContext`. Keep the silent exits for missing inputs and the 10-second timeout in `hooks/hooks.json`. *(deviation: executed as written, then reworked on the user's direction after PR review — the checks moved to `hooks/session-start.mjs` behind a bash wrapper that finds node, and an input that exists but cannot be parsed now fails loud instead of exiting silently; silence remains only for inputs absent by design.)*

## Verification

- Exercise the hook against scratch fixture directories covering: no state file (silent); everything matching (silent); plugin pinned ≠ installed (sync message); plugin pin ahead of `scaffoldedAgainst` (update message); a pack pin ahead of its watermark (update message); a pinned pack missing from `node_modules/` (sync message); a pack installed at a version other than its pin (sync message); two findings at once (one aggregated message); q's own checkout shape, with no pin file (silent). *(deviation: superseded by the rework recorded at Decision 10 — every failure now emits the one uniform `/q:sync` message, stopping at the first, so the per-check routing and aggregation cases no longer exist; a missing state file now fails rather than staying silent. The matrix ran as written before the rework, then grew to 22 scenarios covering the uniform behavior plus corrupt and malformed inputs, hand-installed packs, orphan entries, and non-doc dependencies.)*
