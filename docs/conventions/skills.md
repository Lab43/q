...
- Validation closes execution: what the run delivers passes the project's checks before it is delivered, and an adversarial review where the contract calls for one. The contract carries the shared procedure; a skill's own text adds only its bindings (lenses, scope, diff).
...
A body instructs the agent executing it: every action gets a command (source: @lab43/q conventions/writing.md, Instructions are commands).
