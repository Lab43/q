...
An exception must name a section, because only a marker that identifies the rule can be counted against it. A spec marker names the doc alone, because it heads the unit enforcing the spec rather than a single statement's check (see: @lab43/q conventions/specs.md, Enforcement). An exception must also carry a reason, which is the text the marker sits in: the sentence in a doc, or the comment in any other file. An exception missing either part excuses nothing.
