...
One feature or domain per doc. Every spec opens with a title and an intro stating the feature's purpose, which is the territory the doc commits to. Sections group commitments. A section heading is what a plan's Context, a source marker, and an amendment cite, so it names the commitment's subject and stays stable across edits. Refine a spec in place. Delete the spec of a retired feature.
...
Mark the code that enforces a spec: the validation that rejects the bad input, a guard in the UI, a database constraint, the tests that prove the rules. The marker names the spec and no section, in that file's own comment syntax (see: @lab43/q conventions/documentation.md, Markers). It sits once, at the head of the unit that enforces the spec. A file enforcing one spec carries the marker at its top. A blank line follows the marker there, so the marker reads as the file's and not the first statement's. A function or a test block that enforces a different spec from the rest of its file carries that spec's marker at its head. Never mark individual checks, and never name the section a check answers to. Rationale: a marker on every check repeats one fact on every line of a block and drowns the code. A file whose section names switch every few lines tells the reader nothing they can use. What a reader of the code wants to know is which spec governs the file. Code that merely relies on the commitment carries nothing. Almost everything downstream relies on a commitment in some way, so marking relying code would put markers everywhere and the count would stop meaning anything. When enforcing and relying are hard to tell apart, ask what happens without the code. Without enforcing code the product can break the commitment. Without relying code it cannot.
