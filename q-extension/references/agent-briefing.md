...
Guides — how to use and operate this project, rather than how to write its code (source: @lab43/q conventions/documentation.md, Taxonomy):
...
- **Drop an extension group with no entries.** A fresh project has no extensions, so those groups arrive with the first one. The q group, the Specs group, and the Guides group are always there. When a project has no specs or no guides yet, that group's list is the single line `- none yet`. Rationale: the heading and its statement of purpose tell a session and its user that the doc type exists and where it goes, which is how the first spec or guide gets written at all.
